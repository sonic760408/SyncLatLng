...
public class Worker implements Runnable, DisposableBean {
    ...
    private void uploadCsv(List<Cookie> cookies) {
        ...
        //post.setHeader("Content-Type", "multipart/form-data; ");
        post.setHeader("Referer", UPLOAD_URL);
        post.setHeader("Cookie", cookie_req);
        //post.setHeader("enctype", "multipart/form-data");

        final HttpParams httpParams = new BasicHttpParams();
        HttpConnectionParams.setConnectionTimeout(httpParams, 30000); //30sec
        ...
        if (file.exists()) {
            if (file.isFile()) {
                log.info(" FIND FILE: my.csv");
            } else {
                log.info(" WRONG FILE  my.csv");
            }
        } else {
            log.info(" NO FIND FILE ON " + currentRelativePath + "my.csv");
        }
        ...
        try {
            ...
            builder.setContentType(ContentType.MULTIPART_FORM_DATA);
            /*
                builder.addTextBody("ADDR_BOX_CRS", COORDINATOR_EPSG_WGS84, ContentType.MULTIPART_FORM_DATA);
                builder.addTextBody("refFile", "MY.csv", ContentType.MULTIPART_FORM_DATA);
                builder.addTextBody("ADDR_BOX_ISONLY_FULL_MATCH", "0", ContentType.MULTIPART_FORM_DATA);
                builder.addTextBody("ADDR_BOX_ODD_EVEN_TYPE", "0", ContentType.MULTIPART_FORM_DATA);
                builder.addTextBody("ADDR_BOX_ODD_OEVEN", "0", ContentType.MULTIPART_FORM_DATA);
                builder.addTextBody("ADDR_BOX_CanIgnoreVillage", "on", ContentType.MULTIPART_FORM_DATA);
                builder.addTextBody("ADDR_BOX_CanIgnoreNeighborhood", "on", ContentType.MULTIPART_FORM_DATA);
                builder.addTextBody("ADDR_BOX_ReturnMaxCount", "0", ContentType.MULTIPART_FORM_DATA);
                builder.setCharset(Charset.forName("UTF-8"));
             */
            //builder.addTextBody("method", "ExecuteBathQuery",ContentType.APPLICATION_FORM_URLENCODED);
